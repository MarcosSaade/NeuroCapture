...
from app.crud.assessment_crud import (
    get_assessments, get_assessment,
...
@router.post(
    "/", response_model=AssessmentInDB, status_code=status.HTTP_201_CREATED
)
async def create_assessment_endpoint(
    ...
    return await create_assessment(db, patient_id, assessment_in)
...
async def update_assessment_endpoint(
    ...
    if not obj or obj.patient_id != patient_id:
        raise HTTPException(status_code=404, detail="Not found")
    return await update_assessment(db, obj, assessment_in)
